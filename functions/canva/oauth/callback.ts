/**
 * Cloudflare Pages Function — GET /canva/oauth/callback
 *
 * Target of the Canva Connect API redirect after a user approves access.
 * Canva lands here with `?code=X&state=Y`.
 *
 * Responsibilities:
 *   1. Extract code + state from the query string.
 *   2. Forward them (server-to-server) to the agent at
 *      ${AGENT_BASE_URL}/api/canva/oauth/receive
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  if (errorCode) {
    return htmlResponse(
      ...
      renderPage({
        ...
        detail: `Canva returned: ${errorCode} — ${errorDetail || "no additional detail"}. If you declined by accident, start over from the consent link.`,
      }),
    );
  }
  ...
  if (!code || !state) {
    return htmlResponse(
      ...
      renderPage({
        ...
        detail: "This URL was hit without the expected code and state parameters. This usually means you opened the callback URL directly instead of completing the Canva consent flow.",
      }),
    );
  }
  ...
  let resultSummary = "";
  try {
    const resp = await fetch(`${base}/api/canva/oauth/receive`, {
      method: "POST",
      headers: {
    ...
    if (resp.ok && (body as { ok?: boolean }).ok) {
      forwardOk = true;
      const accountRef = String((body as any).account_ref || "unknown");
      const userId = String((body as any).canva_user_id || "");
      const scopes = (body as any).scopes;
      const scopeSummary = Array.isArray(scopes) ? `${scopes.length} scopes` : "scopes ok";
      resultSummary = `Token stored · account_ref=${accountRef}${userId ? ` · canva_user_id=${userId}` : ""} · ${scopeSummary}`;
    } else {
  ...
  return htmlResponse(
    ...
    renderPage({
      ok: forwardOk,
      title: forwardOk ? "Canva authorization complete" : "Authorization failed",
...
<html lang="en"><head>
  ...
  <div class="card">
    ...
    <h1>${safeTitle}</h1>
    <p>${safeDetail}</p>
    <p class="small">Glitch Grow · Canva integration · <code>grow.glitchexecutor.com/canva/oauth/callback</code></p>
  </div>
</body></html>`;
}
