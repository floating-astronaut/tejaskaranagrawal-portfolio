/**
 * Cloudflare Pages Function — GET /tiktok/oauth/callback
 *
 * Target of the TikTok Business redirect after a user approves access.
 * TikTok lands here with `?auth_code=X&state=Y`.
 *
 * Responsibilities:
 *   1. Extract auth_code + state from the query string.
 *   2. Forward them (server-to-server) to the agent at
 *      https://insights.glitchexecutor.com/api/tiktok/oauth/receive
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  if (errorCode) {
    return htmlResponse(
      ...
      renderPage({
        ...
        detail: `TikTok returned: ${errorCode} — ${errorDetail || "no additional detail"}. If you declined by accident, start over from the consent link.`,
      }),
    );
  }
  ...
  if (!authCode || !state) {
    return htmlResponse(
      ...
      renderPage({
        ...
        detail: "This URL was hit without the expected auth_code/code and state parameters. This usually means you opened the callback URL directly instead of completing the TikTok consent flow.",
      }),
    );
  }
  ...
  let resultSummary = "";
  try {
    const resp = await fetch(`${base}/api/tiktok/oauth/receive`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${secret}`,
      },
      body: JSON.stringify({ code: authCode, auth_code: authCode, state }),
    });
    const body = await resp.json().catch(() => ({} as Record<string, unknown>));
    if (resp.ok && (body as { ok?: boolean }).ok) {
      forwardOk = true;
      resultSummary = `Token stored · account_ref=${String((body as any).account_ref || "unknown")} · advertisers=${String((body as any).advertiser_count || 0)}`;
    } else {
  ...
  return htmlResponse(
    ...
    renderPage({
      ok: forwardOk,
      title: forwardOk ? "TikTok authorization complete" : "Authorization failed",
      detail: forwardOk ? `You can close this tab. ${resultSummary}` : resultSummary,
    }),
  );
};
...
<html lang="en"><head>
  ...
  <title>TikTok Ads · ${safeTitle}</title>
  <style>
  ...
  <div class="card">
    ...
    <h1>${safeTitle}</h1>
    <p>${safeDetail}</p>
    <p class="small">Glitch Grow · TikTok Ads integration · <code>grow.glitchexecutor.com/tiktok/oauth/callback</code></p>
  </div>
</body></html>`;
}
